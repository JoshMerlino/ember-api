...
export async function lookupHost(host: string, user: User): Promise<Host> {
	...
	if (!user.meta.subscription) throw new Error("User does not have a subscription");

	// Validate subscription
	const subscription = await stripe.subscriptions.retrieve(user.meta.subscription);
